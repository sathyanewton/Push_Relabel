  ...
  for(i=(s+1);i<t;i++)
  {
    ...
    p.excess[i]=0;
      ...
      for(l=0;l<p.G[s].size();l++)
      {
            ...
            {
            cout<<"node "<<p.G[s][l].to<<"\n";
            p.returnexcess[l].push_back(p.G[s][l].to);
            }      
      }  
...
cout<< "excess[s] before while :"<<p.excess[s]<<" excess[t] before while :"<<p.excess[t]<<"\n\n\n";
  ...
  int satsat=0,bbb=0;
   ...
   {
   	/*cout<<"source push"<<"\n";
        for(k=0; k<=p.returnexcess[s].size();k++)     
       {                       
        p.active[p.returnexcess[s][k]]=1;   // push 
        cout<<"p.returnexcess[s][k] : "<<p.returnexcess[s][k]<<"\n";
        privQ.push(p.returnexcess[s][k]);
       }*/
cout<<"*ant : "<<*ant<<"\n";
cout<<"nodes adjacent to source : "<<*(ant)<<"\n";
privQ.push(*(ant));
/*
       for(k=1; k<=*ant;k++)     
       {                        
        p.active[*(ant-k)]=1;   // push 
        cout<<"nodes adjacent to source : "<<*(ant-k)<<"\n";
        privQ.push(*(ant-k));
       }
       */
   }
...
thread((void *) &p.returnexcess[0][p.returnexcess[0].size()-1]);
