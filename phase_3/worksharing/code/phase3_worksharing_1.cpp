...
working[thread_id]=1;
  while((p.excess[s]+p.excess[t])<0)
  {
    ...
    if(privQ.empty()==true && flag==1)
    {
     ...
     for(i=1;i<(NoOfThreads+1);i++)
     {
    pthread_mutex_lock(&mutex4);
      if(working[i]==1 && i!=thread_id && dirty[i]==0 && request[i]!=1)
        { 
            request[i]=1;
     pthread_mutex_unlock(&mutex4);
            cout<<"request mmmmmmmmmmmmmmmmm of thread : "<<i<<" is set "<<"\n";
            pthread_mutex_lock(&mutex5);
            /*use conditional variable here*/
            while(global_worksharing_buffer.empty()==true)
            {
                pthread_cond_wait(&cond,&mutex5);
            }
            rc = global_worksharing_buffer.size();
            cout<<"vertex exchange happeneing in thread : "<<thread_id<<"\n";
            for(m=0;m<rc;m++)
            {
