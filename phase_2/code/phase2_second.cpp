...
vector<int*>returnexcess1 = *reinterpret_cast<vector<int>*>(&tp);

for(i=0;i<returnexcess1.size();i++)
{
  cout<<"i value : "<<i<<" -> "<<returnexcess1[i]<<"\n";
}
...
for(i=0;i<NoOfThreads;i++)
{
 ...
 for(j=0;j<(p.returnexcess[i].size()-1);j++)
 {
  ...
  cout<<" i value :"<<i<<"j value : "<<j<<"return excess value"<<p.returnexcess[i][j];
      z.push_back(p.returnexcess[i][j]);
      cout<<"z value : "<<z[j]<<"\n";
 }
 cout<<"\n";  
cout<<"calling thread create for thread-id : "<<i<<"\n";
pthread_create(&t[i], NULL,thread,(void *) &z);
}
