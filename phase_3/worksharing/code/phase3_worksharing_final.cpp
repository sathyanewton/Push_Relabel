...
pthread_mutex_lock(&mutex4);
            ...
            while(global_worksharing_buffer.empty()==true && (p.excess[s]+p.excess[t])<0)
            {
                pthread_cond_wait(&cond,&mutex4);
                
                if((p.excess[s]+p.excess[t])>=0)
                {
                  global_worksharing_buffer.clear();
                  cout<<"recived condition from exit state-->thread id : "<<thread_id<<"\n";
                  break;
                }
...
cout<<"excess[s] "<<p.excess[s]<<"excess[t] "<<p.excess[t]<<"in thread "<<thread_id<<"\n";

pthread_mutex_lock(&mutex1); 
cout<<"master count : "<<master_count<<" thread id : "<<thread_id<<"\n"; 
cout<<"number of requests raised : "<<number_of_requests<<"thread id : "<<thread_id<<"\n";
cout<<"wc count : "<<wc<<" in thread : "<<thread_id<<"\n";
for(k=0;k<wc;k++)
{
    pthread_cond_signal(&cond);
}
