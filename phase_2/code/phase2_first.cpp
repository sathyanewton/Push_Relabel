...
void *thread(void *tp)
{
  ...
  t=tp1->y;         //y = t
  tq = tp1->tno;
  vector <int> returnexcess1;
  cout<<"tp1's size : "<<tp1->z.size()<<"\n";
  for(i=0;i<(tp1->z.size()-1);i++)
  {
    cout<<"i value : "<<i<<"returnexcess array :"<<tp1->z[i]<<"thread number : "<<tq<<"\n";
    returnexcess1.push_back(tp1->z[i]);
  }
  cout<<"after checkpoint"<<"\n";
   //main while
while(1);
cout<<"inside thread :"<<tq<<"excess[s] before while :"<<p.excess[s]<<"excess[t] before while :"<<p.excess[t]<<"\n";
  ...
  int satsat,bbb;
   ...
   if(flag==0)
   {
      ...
      for(k=0;k<(returnexcess1.size()-1);k++)
      {
        p.active[returnexcess1[k]]=1;
        cout<<"\n active values of node "<<returnexcess1[k]<<" is "<<p.active[returnexcess1[k]];
        privQ.push(returnexcess1[k]);
      }
   }
...
{
 args.tno=i;
 cout<<"p.returnexcess[i].size()-1 "<<p.returnexcess[i].size()-1<<"\n";
 for(j=0;j<(p.returnexcess[i].size()-1);j++)
 {
  cout<<" i value :"<<i<<"j value : "<<j<<"return excess value"<<p.returnexcess[i][j];
    args.z.push_back(p.returnexcess[i][j]);
 }
 cout<<"\n";  
cout<<"calling thread create for thread-id : "<<i<<"\n";
