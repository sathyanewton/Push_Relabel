      ...
      for(l=0;l<p.G[s].size();l++)
      {
        //cout<<"inside main chunk of parallel_initialize\n";
           ...
           // cout<<"node "<<p.G[s][l].to<<" going to thread "<<l%NoOfThreads<<"\n";
            ...
            /*Basic round-robin schdulerof adjecent nodes to the source across number of threads that are there.*/
            }       
      }  
   ...
   if(flag==0)
   {
      cout<< "inside flag==0 loop"<<"\n";
       ...
       {                        //
        ...
        privQ.push(*(ant-k));
       // cout<<"whats in the queue : "<<privQ.front()<<"of thread :"<<tid<<"\n";
       }
   }
        ...
        else              //  not the first push operation and excess[node] is greater than 0
        {
         ...
         // cout << "Excess of node "<<privQ.front()<< " is "<<excess_t<<"\n";
          ...
          for (i=0; i<p.G[privQ.front()].size(); i++)   
            ...
            //  cout<<"node : "<<satsat<<" thread id : "<<(tid/100000000000000)<<"\n";
...
//cout<<"p.returnexcess[i][p.returnexcess[i].size()-1] -> "<<p.returnexcess[i][p.returnexcess[i].size()-1]<<"\n";
