...
//cout<<"inside thread : "<<tid<<" excess[s] before while :"<<p.excess[s]<<"excess[t] before while :"<<p.excess[t]<<"\n";
 ...
 //  cout<<"inside while in thread "<<u<<"\n";
  // cout <<"Excess of s is "<<p.excess[s]<<" thread id "<<thread_id<<"\n";
  // cout<<"Excess of t is "<<p.excess[t]<<" thread id "<<thread_id<<"\n";
   if(flag==0)
   {
...
//        cout<<"nodes adjacent to source : "<<*(ant-k)<<"in thread : "<<thread_id<<"\n";
        /*check for sink*/
          if(*(ant-k)==t)
          {
          cout<<"red alert!!!!!!!!!\n\n\n\n";
...
cout<<"master_count : "<<master_count<<" thread id : "<<thread_id<<"\n";
pthread_exit(NULL);
cout<<"exiting thread"<<"\n";
}         //end threadS
